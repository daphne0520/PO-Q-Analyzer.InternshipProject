    ...
    for table in tables:
        for row in table:
            ...
            for idx, cell in enumerate(cells):
                if str(cell).strip().lower() in ("qty", "quantity"):
                    # header row found; look at the row(s) below for a match position
                    continue
        header_row = table[0] if table else None
        if header_row:
            norm_header = [str(c).strip().lower() if c else "" for c in header_row]
            if "qty" in norm_header:
                qty_idx = norm_header.index("qty")
                for row in table[1:]:
                    if qty_idx < len(row) and row[qty_idx]:
                        try:
                            return float(str(row[qty_idx]).replace(",", ""))
                        except ValueError:
                            continue
    return None
